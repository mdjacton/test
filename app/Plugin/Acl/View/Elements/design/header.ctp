...
<div id="plugin_acl">
	...
	if(!isset($no_acl_links))
	{
        ...
        $links[] = $this->Html->link(__d('acl', 'Actions'), '/admin/acl/acos/index', array('class' => ($selected == 'aros' )? 'selected' : null));
        $links[] = $this->Html->link(__d('bookings', 'List bookings'), '/bookings/index', array('class' => ($selected == 'aros' )? 'selected' : null));
        $links[] = $this->Html->link(__d('users', 'Logout'), '/users/logout', array('class' => ($selected == 'acos' )? 'selected' : null));
