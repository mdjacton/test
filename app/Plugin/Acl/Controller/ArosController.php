...
class ArosController extends AclAppController
{
	...
	function admin_update_user_group()
	{
	    ...
	    {
	        $this->Session->setFlash(__d('acl', 'The user group has been updated'), 'flash_message', null, 'plugin_acl');
	    }
	    else
	    {
	        $errors = array_merge(array(__d('acl', 'The user group could not be updated')), $this->{$user_model_name}->validationErrors);
			...
			if($controller_name != 'App')
			{
    		    foreach($groups as $group)
    	    	{
    	            ...
    	            if(!empty($aro_node))
    	            {
    	        	    ...
    	        	    if(!empty($aco_node))
    	        	    {
    	        	    	$authorized = $this->Acl->check($group, 'controllers/' . $full_action);
	...
	function admin_grant_all_controllers($group_id)
	{
        ...
        if(empty($node))
        {
            ...
            $this->Session->setFlash(sprintf(__d('acl', "The group '%s' does not exist in the ARO table"), $asked_group['Group'][Configure :: read('acl.aro.group.display_field')]), 'flash_error', null, 'plugin_acl');
        }
        else
        {
            //Allow to everything
	...
	function admin_get_group_controller_permission($group_id)
	{
        ...
        if(!empty($aro_node))
        {
	        ...
	        foreach($controller_actions as $action_name)
	        {
        	    ...
        	    if(!empty($aco_node))
        	    {
        	        $authorized = $this->Acl->check($group_data, 'controllers/' . $aco_path);
